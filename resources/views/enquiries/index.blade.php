        ...
        <ul class="mb-4 list-group pt-4">
            @foreach($dates as $date => $enquiries)
                ...
                @foreach ($enquiries as $enquiry)
                    <li class="list-group-item">{{ $enquiry->full_name }} | {{ $enquiry->email }} | £{{ number_format( (int) $enquiry->total_cost, 2) }} | {{ str_before($enquiry->job_type, '|') }} | {{ str_before($enquiry->job_value, '|') }} {{ $enquiry->website_page ? " | $enquiry->website_page" : "" }}</li>
                @endforeach
            @endforeach
        </ul>
